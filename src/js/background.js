          ...
          chrome.windows.create(
            {
              ...
              type: "popup",
              focused: true,
              width: 728, // min constraints for plex popup to look good
              height: 790,
            },
            (win) => {
              let now = new Date().getTime();
              var popupHandle = setInterval(async () => {
                // default pincode expire timeout is 1800
                // can be controlled via plexLoginTimeout
                if (new Date().getTime() - now >= PlexLoginTimeout) {
                  clearInterval(popupHandle);
                  chrome.windows.remove(win.id);
                  // failed to authenticate the user in time
                  // show error message that we timed out
                  makeErrorResponse(
                    `Plex login timed out after ${PlexLoginTimeout / 1000} secs`
                  );
                  return;
                }
              }, 1000);
              chrome.windows.onRemoved.addListener(async (winID) => {
                if (win.id == winID) {
                  // get authToken
                  let response = await plexGetAuthToken(
                    resp["id"],
                    resp["code"]
                  );
                  console.debug("Plex authToken response", response);
